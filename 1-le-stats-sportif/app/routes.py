...
def get_num_jobs():
    """Returnează numărul total de joburi."""
    logger.info("Am intrat in get_num_jobs")
    count = 0
    for job_id in webserver.jobs_status:
        if webserver.jobs_status[job_id] == 'running':
            count += 1
    logger.info("Am iesit din get_num_jobs")
